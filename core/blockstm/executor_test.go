...
type testExecTask struct {
	txIdx        int
	ops          []Op
	readMap      map[STMKey]ReadOperation
	writeMap     map[STMKey]WriteOperation
	sender       common.Address
	nonce        int
	dependencies []int
}

type PathGenerator func(addr common.Address, i int, j int, total int) STMKey
...
func NewTestExecTask(txIdx int, ops []Op, sender common.Address, nonce int) *testExecTask {
	return &testExecTask{
		txIdx:        txIdx,
		ops:          ops,
		readMap:      make(map[STMKey]ReadOperation),
		writeMap:     make(map[STMKey]WriteOperation),
		sender:       sender,
		nonce:        nonce,
		dependencies: []int{},
	}
...
func (t *testExecTask) Execute(mvh *MVHashMap, incarnation int) error {
	...
	version := Version{TransactionIndex: t.txIdx, Incarnation: incarnation}

	t.readMap = make(map[STMKey]ReadOperation)
	t.writeMap = make(map[STMKey]WriteOperation)
	...
	for i, op := range t.ops {
		...
		case readType:
			...
			if i == 0 && val != nil && (val.(int) != t.nonce) {
				return ExecutionAbortError{}
			}

			if result.Status() == MVReadDependency {
				if result.dependencyIdx > deps {
					deps = result.dependencyIdx
				}
			}

			var readKind int

			if result.Status() == MVReadDone {
				readKind = ReadFromMVHashMap
			} else if result.Status() == MVReadNone {
				readKind = ReadFromState
			}

			sleep(op.duration)

			t.readMap[k] = ReadOperation{k, readKind, Version{TransactionIndex: result.dependencyIdx, Incarnation: result.incarnation}}
		case writeType:
			t.writeMap[k] = WriteOperation{k, version, op.val}
		case otherType:
			sleep(op.duration)
		default:
	...
	if deps != -1 {
		return ExecutionAbortError{deps, fmt.Errorf("Dependency error")}
	}

	return nil
}

func (t *testExecTask) MVWriteList() []WriteOperation {
	return t.MVFullWriteList()
}

func (t *testExecTask) MVFullWriteList() []WriteOperation {
	writes := make([]WriteOperation, 0, len(t.writeMap))
...
func (t *testExecTask) MVReadList() []ReadOperation {
	reads := make([]ReadOperation, 0, len(t.readMap))
...
var randomPathGenerator = func(sender common.Address, i int, j int, total int) STMKey {
	return NewStateKey(common.BigToAddress((big.NewInt(int64(i % 10)))), common.BigToHash((big.NewInt(int64(total)))))
}

var dexPathGenerator = func(sender common.Address, i int, j int, total int) STMKey {
	if j == total-1 || j == 2 {
...
func taskFactory(numTask int, sender Sender, readsPerT int, writesPerT int, nonIOPerT int, pathGenerator PathGenerator, readTime Timer, writeTime Timer, nonIOTime Timer) ([]ExecutionTask, time.Duration) {
	exec := make([]ExecutionTask, 0, numTask)
...
func checkNoStatusOverlap(pe *ParallelExecutor) error {
	...
	for _, tx := range pe.execTasks.completed {
		seen[tx] = "complete"
	}
...
// nolint: unparam
func runParallel(t *testing.T, tasks []ExecutionTask, validation PropertyCheck, metadata bool) time.Duration {
	t.Helper()

	profile := false

	start := time.Now()
	result, err := executeParallelWithCheck(tasks, validation, numProcs, nil)
	...
	finalWriteSet := make(map[STMKey]time.Duration)
...
func runParallelGetMetadata(t *testing.T, tasks []ExecutionTask, validation PropertyCheck) map[int]map[int]bool {
	t.Helper()

	res, err := executeParallelWithCheck(tasks, validation, numProcs, nil)
...
func TestBreakFromCircularDependency(t *testing.T) {
	...
	_, err := ExecuteParallel(tasks, numProcs, ctx)
